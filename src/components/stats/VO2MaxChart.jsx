...
export default function VO2MaxChart({ data }) {
  if (!data?.length) return (
    <div className="h-40 flex items-center justify-center text-zinc-600 text-sm">No data yet</div>
  );

  const chartData = data.map((d) => ({ date: fmtDate(d.logged_at), value: parseFloat(d.vo2_max) })).filter((d) => !isNaN(d.value));
    ...
    <ResponsiveContainer width="100%" height={180}>
      <LineChart data={chartData} margin={{ top: 8, right: 8, left: -20, bottom: 0 }}>
        ...
        <Tooltip
          ...
          formatter={(v) => [`${v} ml/kg/min`, "VO2 Max"]}
        />
        <ReferenceLine y={48} stroke="#3b82f6" strokeDasharray="4 4" label={{ value: "48 target", fill: "#3b82f6", fontSize: 10, position: "right" }} />
