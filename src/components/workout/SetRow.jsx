...
export default function SetRow({ set, index, onChange, onDelete }) {
  ...
  return (
    <div className="flex items-center gap-2 py-2">
      <span className="text-xs text-zinc-500 w-5 text-center flex-shrink-0">{set.set_number}</span>
      <input
        type="number"
        placeholder="Reps"
        value={set.reps}
      ...
      />
      <button onClick={() => onDelete(index)} className="text-zinc-600 hover:text-red-400 transition p-1.5 tap-target">
        <Trash2 size={14} />
      </button>
    </div>
  );
